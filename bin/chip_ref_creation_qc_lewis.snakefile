...
#Starting assumes a pre-specifified HD and F250 .bed, .bim, and .fam files
rule ref_build_start:
	input:
		ped = expand("{prefix}{{sample}}.{suffix}", prefix = config["gt_path"], suffix = ["bed", "bim", "fam"])
	params:
		...
		psrecord = "log/{run_name}/psrecord/ref_build_start/ref_build_start.{sample}.log"
	output:
	...
	shell:
		"""
		module load plink
		psrecord "plink --bfile {params.inprefix} --cow --not-chr 0 --threads {params.threads} --memory {params.mem} --make-bed --real-ref-alleles --out {params.oprefix}" --log {params.psrecord} --include-children --interval 5
		"""
...
rule filter_monomorphic:
	...
	params:
		...
		psrecord = "log/{run_name}/psrecord/filter_monomorphic/filter_monomorphic.{sample}.log"
	output:
		bed=temp("imputation_runs/{run_name}/monomorphic_filtered/{sample}.bed"),
		bim="imputation_runs/{run_name}/monomorphic_filtered/{sample}.bim",
		fam="imputation_runs/{run_name}/monomorphic_filtered/{sample}.fam",
		log="imputation_runs/{run_name}/monomorphic_filtered/{sample}.log"
	shell:
		"""
		module load plink
		psrecord "plink --bfile {params.inprefix} --cow --threads {params.threads} --memory {params.mem} --real-ref-alleles --nonfounders --mac {params.mac} --make-bed --out {params.oprefix}" --log {params.psrecord} --include-children --interval 5
		"""
...
# 	shell:
# 		"plink --bfile {params.inprefix} --cow --memoryory 500 --real-ref-alleles --impute-sex ycount --make-bed --out {params.oprefix}; python bin/missexed_animals_filter.py {output.sexcheck} {output.txt}"# python bin/missexed_animals_filter_logging.py {output.txt} {output.log} {params.csv}" #{params.logprefix}.csv"
#
...
# 	shell:
# 		"plink --bfile {params.inprefix} --cow --memoryory 500 --real-ref-alleles --remove {input.txt} --make-bed --out {params.oprefix}"
...
rule merge_assays:
	input:
		expand("imputation_runs/{{run_name}}/monomorphic_filtered/{sample}.bed", sample = config["sample"]),
		expand("imputation_runs/{{run_name}}/monomorphic_filtered/{sample}.bim", sample = config["sample"]),
		expand("imputation_runs/{{run_name}}/monomorphic_filtered/{sample}.fam", sample = config["sample"]),
		expand("imputation_runs/{{run_name}}/monomorphic_filtered/{sample}.log", sample = config["sample"])
	params:
		oprefix="imputation_runs/{run_name}/merged_files/{run_name}",
		pfiles = "imputation_runs/{run_name}/monomorphic_filtered",
		threads = config["plink_threads"],
		mem = config["plink_mem"],
...
rule make_phasing_vcf_extract_lists:
	input:
		bim=expand("imputation_runs/{{run_name}}/monomorphic_filtered/{sample}.bim", sample=config["ref_assays"]),
		fam=expand("imputation_runs/{{run_name}}/monomorphic_filtered/{sample}.fam", sample=config["ref_assays"])
	params:
